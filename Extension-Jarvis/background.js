...
function handleMainHost(response) {
  ...
  if (response.action === 'start_noise') {
    ...
    Protocols.CommunicationProtocols.sendMessageToSpecificTab(response);
  }
  ...
  if (message.action) {
    if (message.receiver === "tab/Conversation GPT") {
      ...
      if (message.action === "send_text_to_chat") {
        Protocols.CommunicationProtocols.sendMessageToSpecificTab(message)
      }
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  try {
    ...
    await Protocols.GeneralProtocols.openNewTab('https://chatgpt.com/c/671e4bf1-5d20-8010-9f8b-91d7ee3149b2', 'tab/Problem Solver GPT');
    // await openNewTab('https://chatgpt.com/c/671e4c21-7ad8-8010-9ad2-619cc347dac3', 'tab/Protocol Generation GPT');
    await Protocols.GeneralProtocols.openNewTab('https://chatgpt.com/c/671b9513-5c58-8010-8a32-4715cd9ae3d7', 'tab/Conversation GPT');
    ...
    Protocols.CommunicationProtocols.sendMessageToSpecificTab(initialize_conversation_message)
    ...
    Protocols.CommunicationProtocols.sendMessageToSpecificTab(initialize_problem_solver_message)
